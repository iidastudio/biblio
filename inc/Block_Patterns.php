...
class Block_Patterns {
	...
	public function __construct() {
		...
		add_action('init', [$this, 'biblio_register_block_pattern_categorys']);
	}
	...
	public function biblio_register_block_pattern_categorys() {
		foreach ( $this->pattern_categorys_to_add as $title => $label ) {
			if ( ! WP_Block_Pattern_Categories_Registry::get_instance()->is_registered($title) ) {
				register_block_pattern_category(
					$title,
					array('label' => __($label, 'biblio'))
				);
			}
		}
	}
